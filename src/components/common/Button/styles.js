...
export const ButtonPassive = styled.div`
  width: 100%;
  min-width: 120px;
  height: 40px;
  display: flex;
  ...
  padding: 0px 20px;
  margin-right: 15px;
  border: 1px solid ${(props) => props.theme.borderButton2};
  border-radius: 5px;
  cursor: pointer;
  transition: 0.2s ease-in all;
  ${(props) => props.theme.type === 'light' && `
  ...
  `}
  ${(props) => props.theme.type === 'dark' && `
    border: 2px solid ${props.theme.borderButton2};
  `}
  &:hover {
    background: ${(props) => props.theme.accessoryHover};
    border: 0px;
...
export const ButtonTextSecondary = styled.div`
  color: ${(props) => props.theme.textButtonSecondary};
  ${ButtonPassive}:hover & {
    color: ${(props) => props.theme.textButton};
  }
`;
